...
Milestone-to-theme mapping matters because this initiative is specifically about connecting short-term execution to long-term direction. Without that mapping, milestone reporting can become a list of disconnected deliverables that are locally meaningful but strategically unclear. Requiring each milestone to point to a larger theme gives the PM a way to ask not only "did work happen?" but also "did this work move the right part of the project forward?" In a research environment where priorities can shift mid-semester, that distinction is important.

---

## 3. Procedures Generated

This report contains one procedure: the **Milestones and Larger Themes Standard Operating Procedure**. Its purpose is to give HAAG Project Managers a complete, reproducible method for running weekly milestone review and monthly larger-theme review using existing team infrastructure. The SOP defines required fields, role responsibilities, cadence, escalation rules, and integration points so the procedure can persist beyond a single semester.

This report includes the full SOP below. A separate standalone copy is also available here: [milestones_and_larger_themes_sop.md](/Users/drake/Documents/GitHub/admin-project-tracking/milestones_and_larger_themes/milestones_and_larger_themes_sop.md)
